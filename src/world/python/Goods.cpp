...
namespace Python
{
struct GoodsWrap: Goods, boost::python::wrapper<Goods>
{
    GoodsWrap(uint64_t id = 0): Goods(id)
    {
    }

    WORLD_PYTHON_WRAP_WORLD_OBJECT(Goods)
};
...
void exportGoods()
{
    ...
    class_<GoodsWrap, bases<Item>, boost::noncopyable> c("Goods", init<uint64_t>());
    c.def(init<>())
